...
import AudioSlicer from "@app/create/components/AudioSlicer";
import { AudioTextContext } from "@app/contexts/AudioTextContext";
import Player from "@app/create/components/Player";
import LlamaButton from "@app/create/components/LlamaButton";
import { transcribeAudioTest } from "@app/utils/speech-to-text";
import Link from "next/link";
import QuestionDisplay from "@app/create/components/QuestionDisplay";
import UploadOneMP3 from "@app/create/components/UploadOneMP3";
import AudioTable from "@app/create/components/AudioTable";
import Transcript from "@app/create/components/Transcript";
import Presentation from "@app/create/components/Presentation";
...
const page = () => {
  ...
  async function getQuestions() {
    const response = await fetch(
      `/api/make-question-json?query=${extractedText}`
    );
    //const response = await fetch(`/api/test`);
    ...
    const contentString = data.replace(
      /(['"])?([a-zA-Z0-9_]+)(['"])?:/g,
      '"$2": '
    );
  ...
  async function getAudioSnippetTimeCodes() {
    const transcriptArray = s2tData.map(
      (element) => element.alternatives[0].transcript
    );
  ...
  return (
    <div style={{ margin: 50 }}>
      ...
      <br></br>
      <button onClick={getAudioSnippetTimeCodes}>
        Get Audio Snippet Time Codes
      </button>
      <br></br>
