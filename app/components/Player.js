...
//make useffect to fetch audio data with audioURL
  async function fetchAudioFile(audioURL) {
    ...
    function sliceAudio() {
        ...
        const slicedData = audioBuffer.getChannelData(0).slice(startSample, endSample);
    ...
    function createSnippetArray() {
        ...
        for (let i = 0; i < snippetTimeStamps.length; i++) {
            ...
            console.log("snippetTimeStamps", snippetTimeStamps[i].start, snippetTimeStamps[i].end);
        }
    }
       ...
       <Grid container direction={"row"} spacing={2} padding={20}>
            <Grid item xs={12} sm={6}>
                ...
                <form onSubmit={handleSubmit}>
                    <h3>Audio Link</h3>
                    <input type="text" value={textValue} onChange={handleURLInputChange} />
                    <button type="submit">Submit</button>
                </form>
                <h1>{audioURL}</h1>
                <h1>OR</h1>
                <input type="file" ref={fileInputRef} onChange={handleAudioFile} accept="audio/*" />
                <div>
                ...
                { audioBuffer ? <button onClick={sliceAudio} >Slice Audio </button> : null}
                { snippetBufferArray ? <button onClick={createSnippetArray} >Create Multi Snippets </button> : null}
               
                <div>
                    <button onClick={playButtonPressedSliced} disabled={!slicedAudioBuffer}>
                        {playing ? "pause": "play"}
                    </button>
                </div>
            </Grid>
