...
const PreReadingVocabSlides = () => {
    ...
    const checkMarkClicked = async (e) => {
        ...
        console.log(checked ? `Checkmark clicked: key ${key}` : `Checkmark unclicked: key ${key}`);
        ...
        if (checked) {
            ...
            const updatedVocabulary = [...vocabulary];
            updatedVocabulary[key] = { ...vocabulary[key], selected: false, img_url: "" };
            updateVocabulary(updatedVocabulary);
            console.log(vocabulary);
        }
    };
  ...
  return (
    <div>
        ...
        <Grid container className="presentation-grid-container" direction={"row"} margin={0}  spacing={0} padding={0} sm={12} style={{ width:"90vw", height:"90%", borderColor: "red"}} >
            <Grid item sm={6} style={{ backgroundColor: "white",borderColor:"purple", borderWidth:2, display:"flex", justifyContent:"center", alignItems:"center", overflow:"auto", maxHeight:500}}>
                <div style={{ width: "90%", height: "80%", borderWidth: 1, borderColor: "green", backgroundColor:"white", padding:20, overflow:"auto" }}>
                    <Grid container direction={"column"}  spacing={0} padding={0} style={{ backgroundColor: "white", paddingLeft:10 }} >
                        { vocabulary ? vocabulary.map((word, index) => (
                        <Grid item xs={12} style={{ borderColor:"red", borderWidth: 1, height:80}}>
                            <Grid container direction={"row"} key={index} sm={12} style={{ marginBottom: 20, borderColor:"green", borderWidth: 1, height:80, }}>
                                <Grid item xs={0.5} sm={1} style={{  height:"100%", display:'flex', alignItems:"center", justifyContent:"center" }} >
                                    <input value={index} checked={word.selected} type="checkbox" style={{ marginLeft: 20, width: 30, height: 30}} onClick={checkMarkClicked}/>
                                </Grid>
                                <Grid item xs={11.5} sm={4}className='flex items-center ' style={{ height:"100%", width:"100%", fontSize: 24,  marginLeft: 10 }}>
                                    <h1 style={{ color: "gray"}}>{word.word + " " + index}</h1>
                                </Grid>
                            </Grid>
                        </Grid>
                        )) : null }
